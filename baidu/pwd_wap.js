...
for (var a = biCompare(d, this.modulus) >= 0; a;) d = biSubtract(d, this.modulus),
...
for (var e = i,
g = t;;) {
    if (0 != (1 & g.digits[0]) && (r = this.multiplyMod(r, e)), g = biShiftRight(g, 1), 0 == g.digits[0] && 0 == biHighIndex(g)) break;
...
if (e == i.length) t = new BigInt;
    ...
    var g = i.length - e,
    s = g % dpl10;
    for (0 == s && (s = dpl10), t = biFromNumber(Number(i.substr(e, s))), e += s; e < i.length;) t = biAdd(biMultiply(t, lr10), biFromNumber(Number(i.substr(e, dpl10)))),
    e += dpl10;
    t.isNeg = r
}
return t
...
var r = new BigInt;
r.digits[0] = t;
for (var e = biDivideModulo(i, r), g = hexatrigesimalToChar[e[1].digits[0]]; 1 == biCompare(e[0], bigZero);) e = biDivideModulo(e[0], r),
digit = e[1].digits[0],
...
for (var t = 0,
r = Math.min(i.length, 4), e = 0; r > e; ++e) t <<= 4,
...
r = i.length,
e = r,
g = 0; e > 0; e -= 4, ++g) t.digits[g] = hexToDigit(i.substr(Math.max(e - 4, 0), Math.min(e, 4)));
return t
}
...
t.isNeg = !t.isNeg;
    ...
    r = new BigInt;
    for (var e, g = 0,
    s = 0; s < i.digits.length; ++s) e = i.digits[s] + t.digits[s] + g,
    r.digits[s] = 65535 & e,
    g = Number(e >= biRadix);
    r.isNeg = i.isNeg
}
...
t.isNeg = !t.isNeg;
    ...
    var e, g;
    g = 0;
    for (var s = 0; s < i.digits.length; ++s) e = i.digits[s] - t.digits[s] + g,
    r.digits[s] = 65535 & e,
    r.digits[s] < 0 && (r.digits[s] += biRadix),
    g = 0 - Number(0 > e);
    if ( - 1 == g) {
        g = 0;
        for (var s = 0; s < i.digits.length; ++s) e = 0 - r.digits[s] + g,
        r.digits[s] = 65535 & e,
        r.digits[s] < 0 && (r.digits[s] += biRadix),
        g = 0 - Number(0 > e);
        r.isNeg = !i.isNeg
...
for (var r, e, g, s = new BigInt,
n = biHighIndex(i), d = biHighIndex(t), a = 0; d >= a; ++a) {
    for (r = 0, g = a, j = 0; n >= j; ++j, ++g) e = s.digits[g] + i.digits[j] * t.digits[a] + r,
...
r = biHighIndex(i),
e = 0;
for (var s = 0; r >= s; ++s) g = result.digits[s] + i.digits[s] * t + e,
...
for (var g = t % bitsPerDigit,
s = bitsPerDigit - g,
n = e.digits.length - 1,
d = n - 1; n > 0; --n, --d) e.digits[n] = e.digits[n] << g & maxDigitVal | (e.digits[d] & highBitMasks[g]) >>> s;
...
s = bitsPerDigit - g,
n = 0,
d = n + 1; n < e.digits.length - 1; ++n, ++d) e.digits[n] = e.digits[n] >>> g | (e.digits[d] & lowBitMasks[g]) << s;
...
s = biNumBits(t),
n = t.isNeg;
if (s > g) return i.isNeg ? (r = biCopy(bigOne), r.isNeg = !t.isNeg, i.isNeg = !1, t.isNeg = !1, e = biSubtract(t, i), i.isNeg = !0, t.isNeg = n) : (r = new BigInt, e = biCopy(i)),
new Array(r, e);
r = new BigInt,
e = i;
for (var d = Math.ceil(s / bitsPerDigit) - 1, a = 0; t.digits[d] < biHalfRadix;) t = biShiftLeft(t, 1),
++a,
++s,
d = Math.ceil(s / bitsPerDigit) - 1;
e = biShiftLeft(e, a),
g += a;
for (var o = Math.ceil(g / bitsPerDigit) - 1, b = biMultiplyByRadixPower(t, o - d); - 1 != biCompare(e, b);)++r.digits[o - d],
e = biSubtract(e, b);
for (var u = o; u > d; --u) {
    ...
    for (var m = r.digits[u - d - 1] * (x * biRadix + c), v = l * biRadixSquared + (h * biRadix + f); m > v;)--r.digits[u - d - 1],
    ...
    e.isNeg && (e = biAdd(e, b), --r.digits[u - d - 1])
}
return e = biShiftRight(e, a),
r.isNeg = i.isNeg != n,
i.isNeg && (r = n ? biAdd(r, bigOne) : biSubtract(r, bigOne), t = biShiftRight(t, a), e = biSubtract(t, e)),
...
g = i,
s = t;;) {
    if (0 != (1 & s.digits[0]) && (e = biMultiplyMod(e, g, r)), s = biShiftRight(s, 1), 0 == s.digits[0] && 0 == biHighIndex(s)) break;
...
for (var r = new Array,
e = t.length,
g = 0; e > g;) r[g] = t.charCodeAt(g),
g++;
for (; r.length % i.chunkSize != 0;) r[g++] = 0;
var s, n, d, a = r.length,
o = "";
for (g = 0; a > g; g += i.chunkSize) {
    for (d = new BigInt, s = 0, n = g; n < g + i.chunkSize; ++s) d.digits[s] = r[n++],
    d.digits[s] += r[n++] << 8;
...
for (r = 0; r < s.length; ++r) {
    var d;
    for (d = 16 == i.radix ? biFromHex(s[r]) : biFromString(s[r], i.radix), g = i.barrett.powMod(d, i.d), e = 0; e <= biHighIndex(g); ++e) n += String.fromCharCode(255 & g.digits[e], g.digits[e] >> 8)
}
...
// function enbaidupwd(pwd) {
// var serverTime = 'e1a6f1d17c';
// var module = '';
// pwd = pwd + serverTime;
// setMaxDigits(131);
// var u = new RSAKeyPair("10001", "", module);
...
function guideRandom() {
        return "xxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(/[xy]/g, function(x) {
